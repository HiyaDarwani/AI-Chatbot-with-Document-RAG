...
// In-memory query cache to avoid redundant API calls on identical/near-identical user questions
...
export async function embedText(text) {
  ...
  if (queryCache.has(cacheKey)) {
    console.log(`[embedder] Cache hit for query: "${cacheKey}"`);
    return queryCache.get(cacheKey);
  }

  if (isMockMode()) {
    const mockEmbedding = Array.from({ length: DIMENSIONS }, (_, i) => Math.sin(i + text.length));
    queryCache.set(cacheKey, mockEmbedding);
