...
Answer based only on the context above. Cite source numbers.`;
  ...
  if (isMockMode()) {
    try {
      ...
      // ── Intent Classifier ──
      const isGreeting = q === 'hi' || q === 'hello' || q === 'hey' || q === 'help';
      const isFollowUp = q.includes('more') || q.includes('yes') || q.includes('continue') || q.includes('go on') || q.includes('explain') || q.includes('pls') || q.includes('please');
      const isSummary = q.includes('summar') || q.includes('what is it') || q.includes('about') || q.includes('overview') || q.includes('explain the doc') || q.includes('what is in the');
      ...
      if (isGreeting) {
        ...
        // Fallback if no specific sentence matched terms
        if (matchedSentences.length === 0) {
          matchedSentences = sentences.slice(0, 3);
        }

        if (matchedSentences.length > 0) {
          answerText = `According to the document context (specifically **Source 1**), here are the details related to your query:\n\n` +
            matchedSentences.map(s => `* ${s}`).join("\n") +
