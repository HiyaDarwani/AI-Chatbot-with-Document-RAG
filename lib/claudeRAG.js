...
Answer based only on the context above. Cite source numbers.`;
  ...
  if (isMockMode()) {
    try {
      ...
      if (matchesGreeting) {
        ...
        // Rotate chunks dynamically based on the follow-up history step to ensure non-repetitive responses
        ...
        if (nextSentences.length > 0) {
          const intros = [
            `Continuing from the next section of the document (**Source ${nextIdx + 1}**):`,
            `Here are some further details found in the subsequent section (**Source ${nextIdx + 1}**):`,
            `Extracting additional context from the next text block (**Source ${nextIdx + 1}**):`
          ];
          const intro = intros[queryCount % intros.length];
          answerText = `${intro}\n\n${nextSentences.join(" ")}\n\nLet me know if you want me to expand on any other parts.`;
        } else {
        ...
        // Compile a dynamic summary starting at different offsets depending on dialogue turns
        ...
        const summaryOffset = (queryCount * 2) % Math.max(1, allSentences.length - 2);
        const slicedSummary = allSentences.slice(summaryOffset, summaryOffset + 3);

        answerText = `Based on a review of the document, here is a summary of the main points:\n\n` +
          slicedSummary.map(s => `• ${s}`).join("\n\n") +
          `\n\nWhat specific part of this summary would you like to dive into?`;
      } else {
        // Keyword-based search simulation with stop-words filtering
        ...
        // Find chunk with most keyword overlaps if terms exist
        if (terms.length > 0) {
        ...
        // Loop through chunk candidates sequentially (starting from matching/rotated index)
        // to find a slice that does not duplicate any previous chat responses
        let foundUnique = false;
        const startChunkIdx = chunks.indexOf(bestChunk);

        for (let attempt = 0; attempt < chunks.length; attempt++) {
          const candidateIdx = (startChunkIdx + attempt) % chunks.length;
          const candidateChunk = chunks[candidateIdx];
          if (!candidateChunk) continue;

          const textContent = candidateChunk.content;
          ...
          for (let sShift = 0; sShift < Math.max(1, sentences.length - 2); sShift++) {
            const offset = (queryCount * 2 + sShift) % sentences.length;
            let sliced = sentences.slice(offset, offset + 3);
            if (sliced.length < 2) {
              sliced = sentences.slice(0, 3);
            }

            const sourceIdx = candidateIdx + 1;
            const intros = [
              `According to the document (**Source ${sourceIdx}**), `,
              `The document context (**Source ${sourceIdx}**) indicates that `,
              `Based on the retrieved sections (**Source ${sourceIdx}**), `,
              `From the text (**Source ${sourceIdx}**), we find that `
            ];
            const intro = intros[(queryCount + attempt + sShift) % intros.length];
            const paragraph = sliced.join(" ");
            const candidateAnswer = `${intro}${paragraph.charAt(0).toLowerCase() + paragraph.slice(1)}\n\nIs there anything specific in this section you want to elaborate on?`;

            // Check if this candidate matches text in any previous answers
            const isDuplicate = previousAnswers.some(prev => 
              prev.includes(sliced[0].toLowerCase().slice(0, 35))
            );

            if (!isDuplicate || (attempt === chunks.length - 1 && sShift === sentences.length - 3)) {
        ...
        answerText = finalAnswer || "I couldn't find any additional direct details in the document. Try asking about a different topic.";
      }
  ...
  try {
    // Build alternating message payload including chat history context for Claude
    ...
    if (Array.isArray(history) && history.length > 0) {
      ...
      messagesPayload.push(...formatted);
    }
