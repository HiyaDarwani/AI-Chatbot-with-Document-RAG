  ...
  if (isMockMode()) {
    try {
      ...
      if (matchesGreeting) {
        ...
        let bestChunk = chunks[queryCount % chunks.length]; // default rotation
        ...
        // Rotate slicing offset on repeat queries
        const sliceOffset = (queryCount * 2) % Math.max(1, sentences.length - 2);
        if (matchedSentences.length === 0) {
          matchedSentences = sentences.slice(sliceOffset, sliceOffset + 3);
        } else {
          matchedSentences = matchedSentences.slice(0, 3);
        }

        if (matchedSentences.length > 0) {
          const sourceIdx = chunks.indexOf(bestChunk) + 1;
          const intros = [
