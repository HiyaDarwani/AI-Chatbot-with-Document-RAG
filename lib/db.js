...
export function isMockMode() {
  ...
  const jinaKey = process.env.JINA_API_TOKEN;
  
  const antKey = process.env.ANTHROPIC_API_KEY;
  const geminiKey = process.env.GEMINI_API_KEY;
  const groqKey = process.env.GROQ_API_KEY;

  const hasDb = url && dbKey && !url.includes('your_') && !url.includes('<') && !dbKey.includes('your_') && !dbKey.includes('<');
  const hasJina = jinaKey && !jinaKey.includes('your_') && !jinaKey.includes('<');
  
  const hasChatKey = (antKey && !antKey.includes('your_') && !antKey.includes('<')) ||
                     (geminiKey && !geminiKey.includes('your_') && !geminiKey.includes('<')) ||
                     (groqKey && !groqKey.includes('your_') && !groqKey.includes('<'));

  return !(hasDb && hasJina && hasChatKey);
}
...
class MockSupabaseClient {
  ...
  async rpc(fn, args) {
    ...
    if (fn === 'match_chunks') {
      ...
      .filter(r => r.similarity >= (args.min_similarity !== undefined ? args.min_similarity : 0.35))
